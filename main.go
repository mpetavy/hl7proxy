...
func init() {
	common.Init(true, "1.2.0", "", "", "2018", "HL7 connection proxy", "mpetavy", fmt.Sprintf("https://github.com/mpetavy/%s", common.Title()), common.APACHE, nil, start, stop, nil, 0)
...
func start() error {
	...
	go func() {
		...
		for common.AppLifecycle().IsSet() {
			...
			outsideDelay := common.NewNotice(true)
